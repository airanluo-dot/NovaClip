(function () {
  ...
  const send = (type, payload) => {
    ...
    try { window.chrome.webview.postMessage({ schemaVersion, type, payload }); } catch (_) { /* The WebView may be closing. */ }
  };
  ...
  function findObject(root, names, depth = 0, seen = new WeakSet()) {
    ...
    if (!Array.isArray(root)) {
      ...
      for (const key of Object.keys(root)) {
        const result = findObject(root[key], names, depth + 1, seen);
        if (result) return result;
      }
    } else {
      for (const item of root) {
        const result = findObject(item, names, depth + 1, seen);
        if (result) return result;
      }
    }
    return null;
  ...
  function readContext() {
    ...
    send("pageContextChanged", { url: location.href, kind, aid, bvid, cid, episodeId, page: pageNumber && pageNumber > 0 ? pageNumber : 1, title, episodeTitle: text(video && video.long_title, null) });
  ...
  function wrapHistory(name) {
    ...
    try { history[name] = wrapped; } catch (_) { /* Some pages expose a read-only history object. */ }
  }
